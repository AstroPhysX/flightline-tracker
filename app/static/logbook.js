...
function draw(data){
  ...
  const coverage=document.getElementById('lb-coverage');if(coverage)coverage.textContent=s.entries?t('map_coverage',{mapped:s.mapped_entries||0,total:s.entries||0,unmapped:s.unmapped_entries||0}):'';document.getElementById('logbook-empty').style.display=s.entries?'none':'block';
  ...
  for(const r of connections){const visual=routeVisual(r.count,maxRouteCount);addRepeatedRoute(r,greatCirclePoints([r.origin_lat,r.origin_lon],[r.destination_lat,r.destination_lon]),{...visual,color:cssVar('--logbook-route','#0071d9')},`${r.origin} ↔ ${r.destination} · ${t('flights_count',{count:r.count})} · ${t('hours_count',{hours:fmt(r.hours,1)})}`,routeDetailsHtml(r));}
  const bounds=[];
  for(const p of data.airports||[]){const radius=Math.min(12,3+Math.log2((p.visits||1)+1)*1.25);for(const offset of[-360,0,360]){const c=L.circleMarker([p.lat,p.lon+offset],{radius,weight:1.8,color:cssVar('--logbook-airport-stroke','#07131f'),fillColor:cssVar('--logbook-airport-fill','#35b9f2'),fillOpacity:.92,className:'logbook-airport-marker'}).addTo(map);c.bindTooltip(`${p.city||p.name||p.code} ${p.code} · ${t('visits_count',{count:p.visits})}`,{sticky:true});c.bindPopup(airportDetailsHtml(p),{maxWidth:360});c.on('click',e=>{if(e.originalEvent)L.DomEvent.stopPropagation(e.originalEvent);focusAirport(p.code);});pointLayers.push(c);pointRecords.push({code:p.code,layer:c,baseRadius:radius});}bounds.push([p.lat,p.lon]);}
